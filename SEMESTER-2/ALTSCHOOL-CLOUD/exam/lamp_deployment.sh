#!/bin/bash
echo "------------------------------------Welcome to 2nd semester exam solution--------------------------------"
...
sudo mysql -uroot -e "CREATE USER 'laraveladmin'@'localhost' IDENTIFIED BY 'laraveladmin123';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON laraveldb.* TO 'laraveladmin'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '26 s/=root/=laraveladmin/' /var/www/laravel/.env
sudo sed -i '27 s/=/=laraveladmin123/' /var/www/laravel/.env
